...
*/
// ==UserScript==
// @name         Enhanced Slither.io Bot - Moving Target Collision Avoidance with Trajectory Projection
// @namespace    https://github.com/enhanced-slither-collision
// @version      4.1.1
// @description  Complete Slither.io Bot with Enhanced Moving Target Collision Avoidance, Mouse Input Blending, and 3-Derivative Trajectory Projection
...
(() => {
  ...
  var Visualizer = class {
    ...
    drawLine(start, end, color, width = 2, alpha = 1) {
  ...
  var CollisionAvoidance = class {
    ...
    showTrajectoryProjection = true; // Show projected trajectory
    trajectoryProjectionTime = 2.0;  // How many seconds to project ahead
    trajectoryProjectionSteps = 20;  // Number of points in projection
    ...
    // Trajectory projection data
    trajectoryHistory = []; // Store last 5 positions and angles
    maxTrajectoryHistory = 5;
    projectedTrajectoryPoints = []; // Calculated projection points
    trajectoryProjectionTime = 2.0; // How many seconds to project ahead
    trajectoryProjectionSteps = 20; // Number of points in projection
    ...
    update(snake, snakes, borderSize) {
      ...
      // Update trajectory history for projection
      this.updateTrajectoryHistory(headPos, headAngle);
    ...
    drawDebugVisuals(headPos, headAngle, snakeRadius) {
      ...
      if (this.showTrajectoryProjection) {
        this.drawProjectedTrajectory();
      }
    ...
    // Draw projected trajectory visualization
    drawProjectedTrajectory() {
      if (!this.projectedTrajectoryPoints || this.projectedTrajectoryPoints.length < 2) {
        return;
      }

      // Draw trajectory history (recent positions)
      if (this.trajectoryHistory.length >= 2) {
        for (let i = 1; i < this.trajectoryHistory.length; i++) {
          ...
          // Draw history line in dark blue
          this.visualizer.drawLine(
            { x: prev.x, y: prev.y },
            { x: curr.x, y: curr.y },
            "darkblue",
      ...
      for (let i = 1; i < this.projectedTrajectoryPoints.length; i++) {
        const prev = this.projectedTrajectoryPoints[i - 1];
        const curr = this.projectedTrajectoryPoints[i];
        
        // Color based on time (fade from bright to dim)
        const timeProgress = i / this.projectedTrajectoryPoints.length;
        const alpha = 0.8 - (timeProgress * 0.6); // Fade from 0.8 to 0.2
        
        // Color based on turn rate
        let color = "lime"; // Default for straight movement
        if (Math.abs(prev.turnRate) > 0.1) {
          color = "yellow"; // Turning
        }
        if (Math.abs(prev.turnRate) > 0.3) {
          color = "orange"; // Sharp turn
        }
        if (Math.abs(prev.turnRate) > 0.5) {
          color = "red"; // Very sharp turn
        }
      ...
      // Draw trajectory points at intervals
      for (let i = 0; i < this.projectedTrajectoryPoints.length; i += 5) {
        const point = this.projectedTrajectoryPoints[i];
        if (point) {
      ...
      // Draw end point of projection
      const endPoint = this.projectedTrajectoryPoints[this.projectedTrajectoryPoints.length - 1];
      if (endPoint) {
        this.visualizer.drawCircle(
          { x: endPoint.x, y: endPoint.y, r: 8 },
          "magenta",
          false,
          0.8
        );
      ...
      // Draw trajectory info text (console output)
      if (this.projectedTrajectoryPoints.length > 0) {
        const currentPoint = this.projectedTrajectoryPoints[0];
        const endPoint = this.projectedTrajectoryPoints[this.projectedTrajectoryPoints.length - 1];
        
        // Only log occasionally to avoid spam
        if (Math.random() < 0.1) { // 10% chance each frame
          console.log(`🎯 Trajectory Projection: Speed=${currentPoint.speed.toFixed(1)}px/s, TurnRate=${(currentPoint.turnRate * 180 / Math.PI).toFixed(1)}°/s, ProjectionTime=${this.trajectoryProjectionTime.toFixed(1)}s, Points=${this.projectedTrajectoryPoints.length}`);
        }
      }
    }
    ...
    // Toggle trajectory projection
    toggleTrajectoryProjection() {
      this.showTrajectoryProjection = !this.showTrajectoryProjection;
      console.log(`🎯 Trajectory Projection: ${this.showTrajectoryProjection ? 'ENABLED' : 'DISABLED'}`);
    }
  ...
  var trajectoryProjectionState = state2(true);
  ...
  var prefOverlay = div(
    ...
    [
      ...
      div({ class: "pref-overlay__item" }, [
        span({ class: "pref-overlay__label" }, "[P] Trajectory Projection: "),
        span(
          { class: getToggleClass(trajectoryProjectionState) },
          getToggleValue(trajectoryProjectionState)
        )
      ]),
      div({ class: "pref-overlay__item" }, [
  ...
  var keyMap = {
    ...
    p: () => {
      collisionAvoidance.toggleTrajectoryProjection();
      trajectoryProjectionState.val = collisionAvoidance.showTrajectoryProjection;
    }
  };
